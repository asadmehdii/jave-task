...
public class LoginAttemptService {
	...
	/**
	 * The max failed login attempts on a given account before it is locked. A value
	 * of 0 will disable locking accounts based on failed logins.
	 */
	@Value("${users.security.failedLoginAttempts}")
	private int failedLoginAttempts;

	/**
	 * The account lockout duration. A value less than 0 means accounts can only be
	 * unlocked by action, not duration. A value of 0 means account
	 * lockouts are disabled. A value greater than 0 is the number of minutes that
	 * an account will stay locked before automatically unlocking.
	 */
	@Value("${users.security.accountLockoutDuration}")
	...
	public void loginFailed(final String email) {
		log.debug("Login attempt failed for email: {}", email);
	...
	public User checkIfUserShouldBeUnlocked(User user) {
		...
		if (user.isLocked() && user.getLockedDate() != null && accountLockoutDuration >= 0) {
			LocalDateTime lockedDate = user.getLockedDate();
			LocalDateTime now = LocalDateTime.now();

			// Calculate duration between locked date and now
			long diffMinutes = ChronoUnit.MINUTES.between(lockedDate, now);
